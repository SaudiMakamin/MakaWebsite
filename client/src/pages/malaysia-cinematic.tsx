...
import { MapPin, Clock, Building2, Globe, Mail, Phone, ExternalLink, Star, Zap, Sparkles, Crown, Shield, Waves, Camera, Rocket, Eye } from 'lucide-react';
...
export default function MalaysiaCinematic() {
  ...
  // Malaysia time update with milliseconds
  useEffect(() => {
    const updateTime = () => {
  ...
  const submitMutation = useMutation({
    mutationFn: async (data: typeof formData) => {
      const response = await apiRequest('POST', '/api/contact', {
        ...data,
        branch: 'Malaysia Office - Cinematic Portal'
      });
      return response.json();
    },
    onSuccess: () => {
      toast({
        title: language === 'ar' ? "✨ تم إرسال الرسالة عبر البوابة الرقمية" : "✨ Message sent through digital gateway",
        description: language === 'ar' ? "سيتواصل معك فريق ماليزيا المتطور قريباً" : "Our advanced Malaysia team will contact you soon",
      });
      setFormData({ name: '', email: '', company: '', subject: '', message: '' });
    },
    onError: () => {
      toast({
        title: language === 'ar' ? "⚡ خطأ في البوابة الرقمية" : "⚡ Digital gateway error",
